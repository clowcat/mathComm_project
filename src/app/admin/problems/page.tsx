...
export default function ProblemManagementPage() {
  ...
  const handleAddRelatedProblem = async (relatedProblem: RelatedProblem) => {
    ...
    // Save to Supabase immediately if connected
    if (isDbConnected) {
      try {
        const savedProblem = await saveProblemToSupabase(newProblem);
        // Replace temp ID with real Supabase ID
        ...
        // Update parent problem's linked_problems in DB
        if (selectedProblem && !selectedProblem.id.startsWith('temp-')) {
          const updatedLinkedProblems = [...linkedProblems, newProblem.id];
          await problemsAPI.update(selectedProblem.id, {
          ...
          try {
            await problemRelationshipsAPI.create(
              selectedProblem.id,  // source (parent problem)
              newProblem.id,       // target (derived problem)
              'derived',           // relationship type
              {
                ...
                strength: 0.8,
                sequenceOrder: problems.filter(p => p.parentProblemId === selectedProblem.id).length
              }
            );
            console.log('✅ Problem relationship created');
          } catch (relError: any) {
        ...
        showToast(`✅ Added "${relatedProblem.title}" and saved to database!`, "success");
      } catch (error: any) {
        showToast(`⚠️ Added locally but DB save failed: ${error.message}`, "error");
      }
    } else {
      showToast(`✅ Added "${relatedProblem.title}" (local only - save parent problem to sync)`, "success");
    }
                  ...
                  <div className="p-4 border border-green-200 rounded-md bg-gradient-to-br from-green-50 to-blue-50">
                    ...
                    <div className="mt-4 p-3 bg-white border border-gray-200 rounded-md">
                      ...
                      <div className="flex gap-2">
                        <Button
                          onClick={async () => {
                            const total = relatedProblems.length;
                            let successCount = 0;
                            
                            for (const prob of relatedProblems) {
                              try {
                                await handleAddRelatedProblem(prob);
                                successCount++;
                              } catch (error) {
                            ...
                            if (successCount === total) {
                              showToast(`✅ Added all ${total} problems to the list!`, "success");
                            } else {
                              showToast(`⚠️ Added ${successCount}/${total} problems (some failed)`, "error");
                            }
                          }}
                          size="sm"
                          className="bg-green-600 text-white font-medium text-xs hover:bg-green-700"
